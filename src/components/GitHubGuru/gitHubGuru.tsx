...
const GitHubGuru: React.FC<GitHubGuruProps> = ({
    ...
    const loadMetaData = async () => {
      try {
        ...
        if (metaData && typeof metaData === 'object') {
          ...
          setAllPullRequests(
            metaData.allPullRequests
              ? (metaData.allPullRequests as GitHubPullRequestWithReviews[])
              : []
          );
          setUserPullRequests(
            metaData.userPullRequests
              ? (metaData.userPullRequests as GitHubPullRequestWithReviews[])
              : []
          );
          previousUserPRsRef.current = metaData.previousUserPRs
            ? (metaData.previousUserPRs as GitHubPullRequestWithReviews[])
            : [];
          setDismissedNotifications(
            new Set(
              metaData.dismissedNotifications ? (metaData.dismissedNotifications as string[]) : []
            )
          );
        }
      } catch (error) {
  ...
  const handlePrClick = useCallback(
    (pr: GitHubPullRequestWithReviews) => {
      if (!isLocked) {
          ...
          <div className={styles.prList}>
            {currentPRs.map((pr) => (
              ...
              >
                <div className={styles.prHeader}>
                  ...
                  <span
                    className={`${styles.prState} ${pr.draft ? styles.prStateDraft : styles.prStateOpen}`}
                  >
                    {pr.draft ? '📝' : '🟢'}{' '}
                    {pr.draft
                      ? t('githubWidget.pullRequests.draft')
                      : t('githubWidget.pullRequests.states.open')}
                  </span>
            ...
            {currentPRs.length === 0 && (
              <div className={styles.noPRs}>
                ...
                <p>
                  {activeTab === 'all'
                    ? t('githubGuru.messages.noPRs')
                    : t('githubGuru.messages.noUserPRs')}
                </p>
              </div>
            )}
          </div>
