...
import {
  dispatchWidgetRemoval,
  widgetEventManager,
  WIDGET_EVENTS,
  WidgetEventListener,
} from '../utils/widgetEvents';
...
const NewTab: React.FC = () => {
  ...
  useEffect(() => {
    const handleMetaUpdated: WidgetEventListener = (event) => {
      ...
      setWidgets((prev) =>
        prev.map((widget) =>
          widget.id === event.widgetId ? { ...widget, metaData: nextMeta } : widget
        )
      );
    };
  ...
  const debouncedSave = useMemo(
    () =>
      debounce(async (widgetsToSave: DashboardWidget[], bgImage: string, locked: boolean) => {
        ...
        try {
          const serializedWidgets: SerializedWidget[] = widgetsToSave.map(
            (widget: DashboardWidget) => {
              // Find the widget type ID for this component
              const componentName =
                widget.component.name || widget.component.displayName || 'unknown';
